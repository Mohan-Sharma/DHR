...
public class UtilDateTime {
	...
	private final static String DATE_TIME_AM_PM_FORMATTER = "dd-MM-yyyy  hh:mm a z Z";
	...
	public static final String[] DAYS = { "Sun", "Mon", "Tue", "Wed", "Thu",
			"Fri", "Sat" };

	public static final String[] DAYSINSHORT = { "SUN", "MON", "TUE", "WED",
			"THU", "FRI", "SAT" };
	...
	public static SimpleDateFormat AM_PM_FORMATTER = new SimpleDateFormat(
			AM_PM_FORMAT);

	public static SimpleDateFormat AM_PM = new SimpleDateFormat("HH:mm");

	public static SimpleDateFormat MM_DD_YYYY = new SimpleDateFormat(
			"MM-dd-yyyy");

	public static SimpleDateFormat HIPHEN_DATE_FORMATTER = new SimpleDateFormat(
			DATE_FORMAT);

	public static SimpleDateFormat MONTH_DATE_FORMATTER = new SimpleDateFormat(
			"MMM, dd");

	public static SimpleDateFormat SLASH_DATE_FORMATTER = new SimpleDateFormat(
			"yyyy/MM/dd");

	public static SimpleDateFormat DEFAULT_DATE_FORMATTER = new SimpleDateFormat(
			"MM/dd/yyyy");

	public static SimpleDateFormat FULL_DATE_FORMATTER = new SimpleDateFormat(
			"MMM,dd,yyyy");
	...
	public static String getIntervalInYearsAndMonths(Date from, Date thru) {
		...
		return (monthDiff / 12) + " Year(s)" + ", " + (monthDiff % 12)
				+ " Month(s)";
	}
	...
	public static int getIntervalInDays(Date from, Date thru) {
		return thru != null ? (int) ((getDayEnd(thru).getTime() - getDayStart(
	...
	public static Date addWeeksToDate(Date start,int weeks){
		GregorianCalendar calendar = new GregorianCalendar();
		calendar.setTime(start);
	...
	public static Date getYearStart(Date stamp, int daysLater, int monthsLater,
			int yearsLater) {
		return getYearStart(stamp, daysLater, monthsLater, yearsLater, TimeZone
				.getDefault(), Locale.getDefault());
	}

	public static Date getYearStart(Date stamp, Number daysLater,
			Number monthsLater, Number yearsLater) {
	...
	public static Date toDate(String monthStr, String dayStr, String yearStr,
		...
		Date newDate = new Date(Integer.parseInt(yearStr), Integer
				.parseInt(monthStr), Integer.parseInt(dayStr), Integer
				.parseInt(hourStr), Integer.parseInt(minuteStr), Integer
				.parseInt(secondStr));
		if (newDate != null) {
			return new Date(newDate.getTime());
		} else {
...
//	}
	 ...
	 * @see java.util.Calendar
	 */
	public static Date adjustDate(Date stamp, int adjType, int adjQuantity,
	...
	public static Date adjustDate(Date stamp, Integer adjType,
			Integer adjQuantity) {
	...
	public static Date getDayEnd(Date stamp, Long daysLater, TimeZone timeZone,
			Locale locale) {
		Calendar tempCal = toCalendar(stamp, timeZone, locale);
		tempCal.set(tempCal.get(Calendar.YEAR), tempCal.get(Calendar.MONTH),
	...
	public static Date getMonthStart(Date stamp, TimeZone timeZone,
			Locale locale) {
	...
	public static Date getYearEnd(Date stamp, TimeZone timeZone, Locale locale) {
		...
		return getMonthEnd(new Date(tempCal.getTimeInMillis()), timeZone,
				locale);
	}
	 ...
	 * DateToString().
	 */
	public static Date stringToDate(String dateTimeString, TimeZone tz,
	 ...
	 * stringToDate().
	 */
	public static String DateToString(Date stamp, String dateTimeFormat,
			TimeZone tz, Locale locale) {
		DateFormat dateFormat = toDateTimeFormat(dateTimeFormat, tz, locale);
	...
	public static Date nowDateOnly() {
		...
		Calendar calendar = Calendar.getInstance();
		calendar.clear();
		calendar.set(temp.get(Calendar.YEAR), temp.get(Calendar.MONTH), temp
...
//			return delta + " Hour(s) ";
		...
		} else if (delta / (24 * 7) < 14)
			return (delta / (24 * 7)) + " Week(s)";
		else if (delta / (24 * 7 * 12) < 20)
			return getIntervalInMonths(dateOfBirth, new Date()) + " Month(s)";
		else
			return getIntervalInYears(dateOfBirth, till) + " Year(s)";
	}
	...
	public static boolean checkAgeRangeInyears(Date dob, Long agefrom,
			Long ageto) {
		if (dob == null)
			return false;
		double ageInYears = UtilDateTime.getIntervalInMonths(dob, new Date()) / 12;
		if ((agefrom != null && ageInYears < agefrom))
			return false;
		if ((ageto != null && ageInYears > ageto))
			return false;
		return true;
	}
	...
	public static Date add(String field, Date date1, int value) {
		if ("Days".equals(field))
			return addDaysToDate(date1, value);
		if ("Weeks".equals(field))
			return addDaysToDate(date1, value * 7);
		if ("Months".equals(field))
			return addMonthsToDate(date1, value);
	...
	public static String format(DateTime dateTime) {
		...
		SimpleDateFormat formatter = new SimpleDateFormat(
				DATE_TIME_AM_PM_FORMATTER);
		formatter.setTimeZone(TimeZone.getTimeZone("GMT"));
		String date = formatter.format(new Date(dateTime.getMillis()));
		return date;
	}
	...
	public static DateTime getDayEnd(DateTime dateTime) {
		...
		Timestamp dayEndTimeStamp = getDayEnd(timestamp, Long.valueOf(0));
		DateTime dayEnd = new DateTime(dayEndTimeStamp.getTime());
		return dayEnd;
	}
	...
	public static DateTime getDayStart(DateTime dateTime) {
		Timestamp timestamp = new Timestamp(dateTime.getMillis());
		Timestamp dayStartTimeStamp = getDayStart(timestamp, 0);
		DateTime dayStart = new DateTime(dayStartTimeStamp.getTime());
		return dayStart;
	}

	public static Timestamp getDayStart(java.sql.Timestamp stamp, int daysLater) {
		return getDayStart(stamp, daysLater, TimeZone.getDefault(), Locale
				.getDefault());
	}
	...
	public static String format(DateTime dateTime, String pattern) {
		...
		String formattedDate = dateFormatter.format(new Date(dateTime
				.getMillis()));
		return formattedDate;
	}
}
